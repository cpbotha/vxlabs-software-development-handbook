...
It is clear that having a well-designed suite of automated testing that can be
executed during development, before features are merged, after features are
merged and especially before releases will ensure that your project or product
works the way that you intend it to.
...
This is a balancing act between fast test cycles times for rapid development
iteration (i.e. developer running tests continuously as they work) vs.  high
confidence that the complete product /functions/ exactly as it should.
...
- coverage is good to monitor, but it can be misleading, e.g. a large suite of
  ...
  - In other words: Measure and study failure models and their impact, create
    tests that target them specifically. In short, apply scientific method when
